...
class CreateAddress extends React.Component {
  constructor(props) {
    ...
    this.focusListener = navigation.addListener('didFocus', ({state, action}) => {
      ...
      Promise.all([
        ...
        this.setState({isLoading: false, telephone:this.props.user.custom_attributes[0]['value']});
  ...
  onClickSave = () => {
    let errors = [];
    const {username, telephone, region_id, city_id, township, street} = this.state;
    if(!username) {
      errors=['username'];
    }
    if(!telephone) {
      ...
      UserService.changeAddress(address)
        .then(() => {
          Promise.all([
            ...
            this.setState({isLoading: false});
            const fromRouter = this.props.navigation.getParam('from', 'MyAddress');
            this.props.navigation.navigate(fromRouter === 'MyAddress' ? 'MyAddress' : 'OrderConfirmation');
          })
        })
        .catch(error => {
          this.props.updateIsForced(false);
    ...
    return (
      <Fragment>
        ...
        <SafeAreaView style={styles.container}>
          <NavHeader navigation={navigation} title={i18n.t('My Address')} />
          <KeyboardAvoidingView
            behavior="padding"
